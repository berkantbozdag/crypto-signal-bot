...
# =========================================================
# RETEST RADAR SCANNER V3
# Teknik + RS uygunsa Telegram'a yollar.
# Orderbook zorunlu değil; iyiyse "destekliyor", kötüyse "teyit yok" yazar.
# =========================================================
...
def build_alert(item):
    ...
    orderbook_ok = bool(ob and ob.get("ok"))

    base_score = tech3d["score"] + tech4h["score"] + rs["score"]
    ob_score = ob["score"] if ob else 0
    total_score = base_score + ob_score

    if orderbook_ok:
        signal_type = "GÜÇLÜ ADAY ✅"
        ob_status = "Orderbook destekliyor ✅"
    else:
        signal_type = "TAKİP ADAYI ⚠️"
        ob_status = "Orderbook teyidi yok / zayıf ⚠️"
    ...
    if book and ob:
        text += (
            f"Orderbook score: <b>{ob['score']}</b>\n"
            f"Detay: {', '.join(ob['reasons'][:4])}\n"
        ...
        text += "Orderbook verisi alınamadı.\n\n"

    if orderbook_ok:
        text += "Yorum: <b>Teknik + RS uygun, orderbook da destekliyor.</b> Yine de retest/reclaim bekle."
    else:
        text += "Yorum: <b>Teknik aday var ama orderbook teyidi zayıf.</b> Sadece takip; piyasa emriyle atlama."
...
def main():
    ...
    for c in stage_hits:
        ...
        try:
            ...
            if ob and book:
                print(
                    f"ORDERBOOK CHECK {symbol}: "
                    f"ok={ob['ok']} "
                    f"score={ob['score']} "
                    f"spread={book['spread_pct']:.3f}% "
                    f"bidask={book['ratio']:.2f}x "
                    f"bid_wall={book['bid_wall_price']:.8g}/{book['bid_wall_usdt']:.0f} "
                    f"bid_wall_dist={book['bid_wall_dist_pct']:.2f}% "
                    f"ask_wall={book['ask_wall_price']:.8g}/{book['ask_wall_usdt']:.0f} "
                    f"bid_depth={book['bid_depth']:.0f} "
                    f"ask_depth={book['ask_depth']:.0f}"
                )
            else:
                print(f"ORDERBOOK CHECK {symbol}: veri yok")

            c["total_score"] = (
                c["tech3d"]["score"]
                + c["tech4h"]["score"]
                + c["rs"]["score"]
                + (ob["score"] if ob else 0)
            )

            c["orderbook_ok"] = bool(ob and ob.get("ok"))
    ...
    final_hits.sort(
        key=lambda x: (
            1 if x.get("orderbook_ok") else 0,
            x.get("total_score", 0),
    ...
    orderbook_confirmed = sum(1 for x in final_hits if x.get("orderbook_ok"))

    header = (
        f"<b>3D + 4H Retest Radar Scanner V3</b>\n"
        f"Saat: {now_utc_str()}\n"
        f"Teknik aday: <b>{len(stage_hits)}</b>\n"
        f"Orderbook onaylı: <b>{orderbook_confirmed}</b>\n"
        f"Gönderilen aday: <b>{len(final_hits)}</b>\n\n"
    )
