...
# =========================================================
# 3D + 4H RETEST / RECLAIM + ORDERBOOK SCANNER
...
# Relative strength
RS_MIN_ADVANTAGE = float(os.getenv("RS_MIN_ADVANTAGE", "3.0"))  # coin BTC'den en az %3 iyi olsun
...
def analyze_3d_structure(bars3d):
    ...
    wider_range_pct = (wider_high - wider_low) / close * 100 if close > 0 else 999
    ...
    # Major 3D direnç: son 2 mumu hariç tutup önceki bölgedeki en yüksek seviye
    if len(bars3d) >= 22:
        resistance_pool = bars3d[-32:-2] if len(bars3d) >= 34 else bars3d[:-2]
    else:
    ...
    major_res = max(b["high"] for b in resistance_pool)
    dist_to_major_res = (close - major_res) / major_res * 100 if major_res > 0 else 999
    ...
    if abs(dist_to_major_res) <= 8:
        score += 2
        reasons.append("3D major direnç yakın")

    if close >= major_res * 0.98:
        score += 1
    ...
    return {
        ...
        "score": score,
        "reasons": reasons,
        "major_resistance_3d": major_res,
        "dist_to_major_res_pct": dist_to_major_res,
...
def analyze_4h_retest(bars4h):
    ...
    # Retest bölgesi: eski direnç etrafında ATR bazlı dinamik bant
    # Çok dar olmasın, çok geniş de olmasın diye yüzde limit koyuyoruz.
    min_buffer = resistance * 0.006     # minimum %0.6
    max_buffer = resistance * 0.035     # maximum %3.5
    ...
    dist_to_res = (close - resistance) / resistance * 100
    ...
    # Retest tutma: fitil bölgeye geliyor ama gövde bölge üstünde/toparlıyor
...
def get_orderbook(symbol):
    ...
    bids = [(fnum(p), fnum(q)) for p, q in data.get("bids", []) if fnum(p) > 0 and fnum(q) > 0]
    asks = [(fnum(p), fnum(q)) for p, q in data.get("asks", []) if fnum(p) > 0 and fnum(q) > 0]
    ...
    bid_zone = [(p, q, p * q) for p, q in bids if bid_floor <= p <= mid]
    ask_zone = [(p, q, p * q) for p, q in asks if mid <= p <= ask_ceiling]
    ...
    bid_wall_dist_pct = (mid - bid_wall_price) / mid * 100 if bid_wall_price > 0 else 999
    ask_wall_dist_pct = (ask_wall_price - mid) / mid * 100 if ask_wall_price > 0 else 999
...
def build_alert(item):
    ...
    text = (
        ...
        f"Yorum: <b>Takip edilebilir setup.</b> Retest bölgesi gelmeden piyasa emriyle atlama."
    )
...
def main():
    ...
    for c in candidates:
        ...
        try:
            ...
            print(
                f"TECH HIT {symbol}: "
                f"3D={tech3d['score']} 4H={tech4h['score']} RS={rs['advantage']:.1f}%"
            )
    ...
    for c in stage_hits:
        ...
        try:
            ...
            if ob and ob["ok"]:
                ...
                print(
                    f"FINAL HIT {symbol}: "
                    f"score={c['total_score']} bidask={book['ratio']:.2f}x"
                )
    ...
    header = (
        f"<b>3D + 4H Retest + Orderbook Scanner</b>\n"
        f"Saat: {now_utc_str()}\n"
