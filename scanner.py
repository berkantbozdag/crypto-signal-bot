...
# Telegram mesajında en fazla kaç coin listelensin?
MAX_TELEGRAM_COIN_LIST = 40

STABLE_OR_FIAT_BASES = {
    "USDC", "FDUSD", "TUSD", "BUSD", "DAI", "USDP", "PAX", "PYUSD",
    "USDE", "SUSDE", "USDS", "USD1", "AEUR", "EURI", "RLUSD",
    "EUR", "TRY", "BRL", "AUD", "GBP", "RUB", "UAH", "USD"
}
...
def make_coin_lines(df, max_rows=40):
    lines = []
    ...
    for _, row in df.head(max_rows).iterrows():
        ...
        ratio_text = "N/A" if pd.isna(ratio) else f"{ratio}x"
        ...
        lines.append(
            ...
            f"Oran: {ratio_text} | "
            f"ATH: {row['ath_body_ratio']} | "
            f"{row['priority']}"
        )

    if len(df) > max_rows:
        lines.append(f"... +{len(df) - max_rows} coin daha")
...
"""
    ...
    if first_run:
        coin_lines = make_coin_lines(scan_simple, MAX_TELEGRAM_COIN_LIST)
        ...
        message = (
            ...
            "📋 <b>ATH-body filtresinden geçen coinler:</b>\n"
            + "\n".join(coin_lines)
        )
        ...
        coin_lines = make_coin_lines(spikes_simple, MAX_TELEGRAM_COIN_LIST)

        message = (
            "🚨 <b>Volume Spike Bulundu</b>\n\n"
            f"Spike kriteri: {VOLUME_SPIKE_RATIO}x ve üstü\n"
            f"Minimum 1H hacim: {fmt_volume(MIN_1H_VOLUME_USDT)} USDT\n"
            f"Spike coin sayısı: <b>{len(spikes_simple)}</b>\n"
            f"Son tarama: {now_utc()}\n\n"
            "🔥 <b>Spike coinler:</b>\n"
            + "\n".join(coin_lines)
        )
        ...
        if SEND_NO_SPIKE_MESSAGE:
            ...
            message = (
                "✅ <b>Tarama bitti. Bu saatte volume spike yok.</b>\n\n"
                f"ATH-body filtresinden geçen coin sayısı: <b>{len(scan_simple)}</b>\n"
                f"Spike kriteri: {VOLUME_SPIKE_RATIO}x\n"
                f"Minimum 1H hacim: {fmt_volume(MIN_1H_VOLUME_USDT)} USDT\n"
                f"Son tarama: {now_utc()}\n\n"
