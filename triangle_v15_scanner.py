...
# ============================================================
# TRIANGLE BREAK V15 - BINANCE 3D SCANNER
# Dosya adı: triangle_v15_scanner.py
# ============================================================
...
# Retest bekleme kapalı.
WAIT_RETEST = False

# Son sinyal tarama ayarı
INCLUDE_LIVE_CANDLE = True
INCLUDE_LAST_CLOSED_CANDLE = True
INCLUDE_PREV_CLOSED_CANDLE = True

# Hacim: liste dışı bırakma yok, sadece LOWVOL etiketi
...
def get_json(url: str, params: Optional[dict] = None, retries: int = 3):
    ...
    raise RuntimeError(f"GET failed: {url} params={params} err={last_error}")
...
def get_24h_quote_volumes() -> Dict[str, float]:
    ...
    for x in data:
        ...
        try:
            out[sym] = float(x.get("quoteVolume", 0.0))
        except Exception:
            out[sym] = 0.0
...
def build_pivots(rows: List[dict], pvt_len: int) -> Tuple[List[Tuple[int, float, int]], List[Tuple[int, float, int]]]:
    highs = []
    lows = []
    n = len(rows)

    for i in range(pvt_len, n - pvt_len):
...
def pivots_available_until(pivots: List[Tuple[int, float, int]], idx: int) -> List[Tuple[int, float]]:
    return [(bar_i, price) for bar_i, price, confirm_i in pivots if confirm_i <= idx]


def has_real_triangle(
    rows: List[dict],
    atr_values: List[Optional[float]],
    piv_hi_all: List[Tuple[int, float, int]],
    piv_lo_all: List[Tuple[int, float, int]],
    idx: int,
    max_bars: int,
) -> bool:
...
def simulate_v15(rows: List[dict]) -> Dict[int, dict]:
    ...
    ema_main = ema(closes, EMA_PERIOD)
    ...
    for idx in range(n):
        ...
        short_break, short_hh, short_meta = check_tri(
            rows, atr_values, ema_ml, sma_old, sma_new, idx, LB_SHORT, False
        )

        mid_break, mid_hh, mid_meta = check_tri(
            rows, atr_values, ema_ml, sma_old, sma_new, idx, LB_MID, True
        )

        long_break, long_hh, long_meta = check_tri(
            rows, atr_values, ema_ml, sma_old, sma_new, idx, LB_LONG, True
        )

        if USE_REAL_TRIANGLE_FILTER:
            s_pivot_ok = has_real_triangle(
                rows, atr_values, piv_hi_all, piv_lo_all, idx, LB_SHORT * 3
            )

            m_pivot_ok = has_real_triangle(
                rows, atr_values, piv_hi_all, piv_lo_all, idx, LB_MID * 2
            )

            l_pivot_ok = has_real_triangle(
                rows, atr_values, piv_hi_all, piv_lo_all, idx, LB_LONG * 2
            )
        ...
        # Pine'da VOL_MULTI <= 1.0 ise volOK hep true oluyordu.
        vm = vol_ma[idx]
        ...
        if pending and not WAIT_RETEST and (idx - pending_bar) >= 2:
            stayed_up = (
        ...
        if pending and WAIT_RETEST and (idx - pending_bar) >= 2:
            retest_pending = True
            retest_start = idx
        ...
        if retest_pending and retest_level is not None:
            if rows[idx]["low"] <= retest_level * 1.02:
                retest_dipped = True

            if (
                retest_dipped
                and rows[idx]["close"] > retest_level
                and rows[idx]["close"] > rows[idx]["open"]
            ):
                retest_confirm = True
                retest_pending = False

            if idx - retest_start >= RETEST_WINDOW_BARS:
                if (not retest_dipped) and rows[idx]["close"] > retest_level:
                    retest_confirm = True

                retest_pending = False

            if rows[idx]["close"] < retest_level * 0.90:
                retest_pending = False

        short_confirmed = (normal_confirm or retest_confirm) and cnt >= COOL_BARS
...
def scan_symbol(symbol: str, quote_volume_24h: float) -> Optional[dict]:
    ...
    # Aynı coinde birden fazla aday varsa tek satır.
    priority = {
        "CLOSED": 1,
        "LIVE": 2,
        "PREV": 3,
...
def build_message(results: List[dict]) -> str:
    header = (
        ...
        "Ayar: V15 ekran ayarları / AL30-AL90-AL100\n\n"
    )
    ...
    for i, r in enumerate(results[:MAX_ALERT_ROWS], 1):
        ...
        line = (
            ...
            f"Narrow: {fmt_num(meta.get('narrow_pct'))}% | "
            f"Touches: {html.escape(str(meta.get('touches', 'na')))} | "
            f"Slope: {fmt_num(meta.get('upper_slope_pct'))}%\n"
...
def main():
    ...
    print(f"Settings: LB={LB_SHORT}/{LB_MID}/{LB_LONG}, cooldown={COOL_BARS}, old/new threshold={NEW_OLD_BAR_THRESHOLD}")
    ...
    for i, sym in enumerate(symbols, 1):
        qv = volumes.get(sym, 0.0)

        try:
            hit = scan_symbol(sym, qv)

            if hit:
                results.append(hit)
                print(f"[HIT] {sym} {hit['label']} {hit['status']} vol={fmt_money(qv)}")
